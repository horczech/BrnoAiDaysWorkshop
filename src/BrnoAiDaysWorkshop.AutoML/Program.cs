using BrnoAiDaysWorkshop.AutoML.DataStructures;
using BrnoAiDaysWorkshop.AutoML.Helpers;
using Microsoft.ML;
using Microsoft.ML.AutoML;
using Microsoft.ML.Data;

namespace BrnoAiDaysWorkshop.AutoML;
...
public class Program
{
    ...
    private static readonly string TrainDataRelativePath = $"{BaseDatasetsRelativePath}/optdigits-train.csv";
    private static readonly string TestDataRelativePath = $"{BaseDatasetsRelativePath}/optdigits-test.csv";
    ...
    private static readonly string ModelRelativePath = $"{BaseModelsRelativePath}/Model.zip";
    ...
    private static void Train(MLContext mlContext) {
        try {
            ...
            var trainData = mlContext.Data.LoadFromTextFile(
                TrainDataPath,
                new[] { new TextLoader.Column(nameof(InputData.PixelValues), DataKind.Single, 0, 63), new TextLoader.Column("Number", DataKind.Single, 64) },
                hasHeader: false,
                separatorChar: ',');
    ...
    /// <summary>Print top models from AutoML experiment.</summary>
    ...
    private static void TestSomePredictions(MLContext mlContext) {
        ...
        trainedModel.GetOutputSchema(modelInputSchema)["Number"].GetKeyValues(ref keyValues);
        var keys = keyValues.Items().ToDictionary(x => (int)x.Value, x => x.Key);

        //InputData data1 = SampleMNISTData.MNIST1;
        var predictedResult1 = predictionEngine.Predict(SampleMNISTData.MNIST1);
        ...
        var predictedResult2 = predictionEngine.Predict(SampleMNISTData.MNIST2);
